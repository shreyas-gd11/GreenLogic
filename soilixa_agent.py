...
from soilixa_models import SoilixaAction, SoilixaObservation


class BaselineSoilixaAgent:
    """Safety-first baseline policy for the typed Soilixa OpenEnv."""

    def __init__(self) -> None:
        self.epsilon = 0.0

    def act(self, observation: SoilixaObservation) -> SoilixaAction:
        thresholds = observation.thresholds

        if observation.soilMoisture > thresholds.high:
            return SoilixaAction("do_nothing")

        if observation.soilMoisture < thresholds.optimalMin:
            return SoilixaAction("water")
        ...
        if fertilizer_window and stable_moisture and observation.cropHealth < 95.0:
            return SoilixaAction("fertilize")
