...
from soilixa_models import CropTask, SoilixaAction, SoilixaObservation, SoilixaState, StepResult, Thresholds
...
class SoilixaEnv:
    ...
    def __init__(self, crop_type: str = "tomato", seed: int | None = DEFAULT_SEED) -> None:
        ...
        self._state: SoilixaState | None = None
    ...
    def step(self, action: str | SoilixaAction) -> StepResult:
        ...
        action_model = SoilixaAction.from_value(action)
    ...
    def _observation(self) -> SoilixaObservation:
        if self._state is None:
            raise RuntimeError("Environment not initialized. Call reset() first.")
        return SoilixaObservation(
            day=self._state.day,
    ...
    def _fertilizer_effect(self, state: SoilixaState) -> float:
        crop = self._get_crop(state.cropType)
        thresholds = crop.thresholds
    ...
    def _score_from_reward(cumulative_reward: float) -> int:
        ...
        return int(SoilixaEnv._clamp(scaled, 0, MAX_FINAL_SCORE))
